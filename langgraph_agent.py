...
# --- LLMs ---
try:
    orchestrator_llm = Ollama(model="llama2")
    orchestrator_llm.invoke("test prompt for orchestrator llm") # Test
    logger.info("Orchestrator LLM (Ollama) initialized successfully.")
except Exception as e:
    logger.error(f"Failed to initialize Orchestrator LLM: {e}. Orchestrator planning will not function.")
    orchestrator_llm = None

try:
    worker_llm = Ollama(model="llama2")
    worker_llm.invoke("test prompt for worker llm") # Test
    logger.info("Worker LLM (Ollama) initialized successfully.")
except Exception as e:
...
def should_continue(state: OrchestratorState) -> str:
    ...
    plan = state.get("plan", [])
    if not plan: 
        logger.info("Decision: No plan exists. Ending.")
        if not state.get("final_result") and not orchestrator_error and not worker_error: 
            state["final_result"] = "Run failed: No plan was generated or available."
        return END 
    
    current_subtask_index = state.get("current_subtask_index", 0)
    if current_subtask_index < len(plan):
        logger.info(f"Decision: Plan has pending tasks ({current_subtask_index + 1}/{len(plan)}). Routing to worker.")
...
def route_after_start(state: OrchestratorState) -> str:
    logger.info(f"--- Conditional Edge: route_after_start ---")
...
if __name__ == "__main__":
    ...
    initial_state: OrchestratorState = {
        "user_goal": "Read the main README.md file, then write its first 10 characters to a new file named 'readme_head.txt', and then commit this new file with the message 'Added readme_head.txt'.",
